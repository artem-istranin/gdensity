...
def test_wavelets():
    db_order = 2
    phi, psi, t_grid = db_scaling_function_and_wavelet(db_order=db_order, level=10, method='binary_interpolation')
    fig, axs = plt.subplots(nrows=1, ncols=2)
    axs[0].plot(t_grid, phi, color='black')
    axs[0].text(0.95, 0.95, '{}$\phi$'.format(db_order), ha='right', va='top', transform=axs[0].transAxes)
    axs[1].plot(t_grid, psi, color='black')
    axs[1].text(0.95, 0.95, '{}$\psi$'.format(db_order), ha='right', va='top', transform=axs[1].transAxes)
    plt.show()

    db_order = 2
    bc_db_collection = bc_db_scaling_functions_and_wavelets(db_order=db_order, level=10)
    for k in range(0, (db_order - 1) + 1):
        left_bc_phi, left_bc_psi, left_x_grid = bc_db_collection['left'][k]
        fig, axs = plt.subplots(nrows=1, ncols=2)
        axs[0].plot(left_x_grid, left_bc_phi, color='black')
        axs[0].text(0.95, 0.95, '$\phi^{left}_{0,k}$ for ' + 'k = {}'.format(k), ha='right', va='top',
                    transform=axs[0].transAxes)
        axs[1].plot(left_x_grid, left_bc_psi, color='black')
        axs[1].text(0.95, 0.95, '$\psi^{left}_{0,k}$ for ' + 'k = {}'.format(k), ha='right', va='top',
                    transform=axs[1].transAxes)
        plt.show()
    for k in range(-db_order, (-1) + 1):
        right_bc_phi, right_bc_psi, right_x_grid = bc_db_collection['right'][k]
        fig, axs = plt.subplots(nrows=1, ncols=2)
        axs[0].plot(right_x_grid, right_bc_phi, color='black')
        axs[0].text(0.95, 0.95, '$\phi^{right}_{0,k}$ for ' + 'k = {}'.format(k), ha='right', va='top',
                    transform=axs[0].transAxes)
        axs[1].plot(right_x_grid, right_bc_psi, color='black')
        axs[1].text(0.95, 0.95, '$\psi^{right}_{0,k}$ for ' + 'k = {}'.format(k), ha='right', va='top',
                    transform=axs[1].transAxes)
        plt.show()
